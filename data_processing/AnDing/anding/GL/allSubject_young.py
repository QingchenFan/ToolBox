...
lp = '/Volumes/QCI/GL/youngSubID.csv'
label = pd.read_csv(lp)
lab = pd.read_csv(lp)['subject']

fp = '/Volumes/QCI/GL/FC_amygdala_vertex_young/*_*FC'
fpbox = glob.glob(fp)
for i in fpbox:
    print('-----------'+i+'-----------')
    mask = i[-7:]
    ...
    if not reshc.empty:
        reshc.to_csv('/Volumes/QCI/GL/allsubject_young/HC_' + mask + '.csv', index=False)
    ...
    if not resmdd.empty:
        resmdd.to_csv('/Volumes/QCI/GL/allsubject_young/MDD_' + mask + '.csv', index=False)
