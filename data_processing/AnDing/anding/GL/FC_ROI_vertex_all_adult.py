...
mark = 'CM_R'  # 1、SF_R  2、SF_L  3、LB_R 4、LB_L  5、CM_R 6、CM_L     MDD_Amygdala_SF_R_FC_CSV
markbox = ['SF_R','SF_L', 'LB_R' ,'LB_L','CM_R','CM_L']
for m in markbox:
    print(m)
    mark = m
    p = '/Volumes/QCI/Data_13_5/bold_hc/*/sub-*-ap*'
    # TODO:
    ...
    for i in box:
        ...
        if '034' not in i :
            continue
        print(i)
        newp = '/Volumes/QCI/GL/FC_amygdala_vertex_adult/HC_Amygdala_'+mark+'_FC/'
        cnewp = '/Volumes/QCI/GL/FC_amygdala_vertex_adult/HC_Amygdala_'+mark+'_FC_CSV/'
        if not os.path.exists(newp):
            os.makedirs(newp)
        if not os.path.exists(cnewp):
            os.makedirs(cnewp)
        ...
        # TODO:
        df.to_csv('/Volumes/QCI/GL/FC_amygdala_vertex_adult/HC_Amygdala_'+mark+'_FC_CSV/' + subId + '_AMGYDALA_'+mark+'_FC.csv')
