...
for i in datafile:
    ...
    for j in range(0, 7):
        ...
        correlation, pvalue = scipy.stats.pearsonr(box[0], box[1])
...
np.savetxt('./dsi_seeg_PvalueFDR_pearson.txt', res_bonferroni[1])
filer = open('./dsi_seeg_Rvalue_pearson.csv', mode='w')
filep = open('./dsi_seeg_Pvalue_pearson.csv', mode='w')
for i in rlist:
    filer.write(str(i))
    filer.write('\n')
